...
const ListVineyards: React.FC = () => {
    const [vineyards, setVineyards] = useState<VineyardType[] | null>();
    const [allvineyards, setAllVineyards] = useState<VineyardType[]>();
    const [selectedVineyard, setSelectedVineyard] = useState<VineyardType>();
    const [vineyardPolygons, setVineyardPolygons] = useState<L.Polygon[] | L.Polygon>([]);
    ...
    const handleDiseaseChange = (value: string) => {
        ...
        if (filteredVineyards) {
            if (filteredVineyards.length !== 0) {
                ...
                setVineyards(null);
                console.log('filteredVineyards.length == 0');
            }
        } else {
            console.log('no filteredVineyards');
        }
    };
    ...
    const handleVineyardChange = (value: string) => {
        ...
        console.log(
            'selected',
            allvineyards?.find((item) => item.id === value),
        );
    ...
    useEffect(() => {
        ...
        if (vineyards) {
            if (!map) {
                if (selectedVineyard) {
                    const coordinates = selectedVineyard?.geometry.coordinates[0][0];
                    map = L.map('map').setView([coordinates[0], coordinates[1]], 19);
                    const polygon: L.Polygon = L.polygon(selectedVineyard.geometry.coordinates[0], {
                        color: 'purple',
                        weight: 3,
                        fillColor: 'pink',
                        fillOpacity: 0.4,
                    }).addTo(map!);
                    setVineyardPolygons(polygon);
                } else {
                    const coordinates = vineyards[0].geometry.coordinates[0][0];
                    map = L.map('map').setView([coordinates[0], coordinates[1]], 19);
        ...
        return () => {
            ...
            if (vineyardPolygons instanceof L.Polygon) {
                vineyardPolygons.remove();
            } else if (vineyardPolygons instanceof Array) {
                        ...
                        >
                            ...
                            {selectedVineyard && (
                                <div>
                                    ...
                                    <p>
                                        <strong>Area Number:</strong> {selectedVineyard.areanumber}
                                    </p>
                                </div>
                            )}
                        </Form>
